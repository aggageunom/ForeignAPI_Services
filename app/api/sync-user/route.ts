...
export async function POST() {
  try {
    ...
    if (tableCheckError) {
      console.error("Supabase table check error:", tableCheckError);

      // 테이블이 없는 경우 구체적인 안내 메시지 제공
      if (
        ...
        return NextResponse.json(
          {
            ...
            details: `테이블 'users'가 존재하지 않습니다. 마이그레이션 파일(supabase/migrations/setup_schema.sql)을 Supabase Dashboard의 SQL Editor에서 실행해주세요.`,
