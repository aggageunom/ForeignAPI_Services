...
- **추가 필터**
  - 반려동물 동반 가능 필터 (토글)
  - 주차 가능 필터 (토글)
    - 주차 정보는 `detailIntro2` API의 `parking` 필드를 확인
    - 주차 가능한 관광지만 필터링하여 표시
...
- 데스크톱: 리스트(좌측) + 지도(우측) 분할 레이아웃
  - 지도 컨테이너는 sticky positioning으로 스크롤 시에도 항상 상단에 고정
  - 지도 높이는 viewport에 맞춰 자동 조정 (`calc(100vh - 9rem)`)
- 모바일: 탭 형태로 리스트/지도 전환
...
│   ├── detail-map.tsx          # 지도
│   ├── navigation-dialog.tsx   # 길찾기 앱 선택 다이얼로그
