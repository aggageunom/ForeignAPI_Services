...
- [ ] `components/tour-filters.tsx` 생성
  ...
  - [ ] "전체" 옵션 제공
- [ ] 필터 상태 관리 (React State 또는 URL Query)
- [ ] 필터링된 결과 표시
- [ ] 페이지 확인 및 Ux 개선
...
- [ ] URL 복사 기능 (클립보드 API)
- [ ] 복사 완료 토스트 메시지
- [ ] Open Graph 메타태그 동적 생성 (`app/places/[contentId]/page.tsx`)
  - [ ] `og:title` (관광지명)
...
- [ ] `bookmarks` 테이블 생성
  - [ ] `id` (UUID, Primary Key)
  - [ ] `user_id` (UUID, Foreign Key → users.clerk_id)
  - [ ] `content_id` (TEXT, 관광지 contentId)
  - [ ] `created_at` (TIMESTAMP)
- [ ] RLS 정책 설정 (개발 중 비활성화 가능)
- [ ] 마이그레이션 실행 및 테스트
...
- [ ] `components/bookmarks/bookmark-button.tsx` 생성
  - [ ] 별 아이콘 (채워짐/비어있음)
  - [ ] 클릭 시 북마크 추가/제거
- [ ] `lib/api/supabase-api.ts` 생성 (또는 기존 파일 확장)
  - [ ] 북마크 추가 함수
  - [ ] 북마크 제거 함수
  - [ ] 북마크 조회 함수
- [ ] 상세페이지에 북마크 버튼 추가
- [ ] Supabase DB 연동
- [ ] 인증된 사용자 확인 (Clerk)
- [ ] 로그인하지 않은 경우 localStorage 임시 저장 또는 로그인 유도
- [ ] 상세페이지에서 북마크 동작 확인
